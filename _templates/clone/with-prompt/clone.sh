...
cp -r "$PWD/src/components/$1/$2" "$PWD/src/components/$1/$3"
...
find "$PWD/src/components/$1/$3" -type f -name "*" | while read FILENAME ; do
	...
	gsed -i "s#${from_component_kebab}#${to_component_kebab}#g" "${NEW_FILENAME}"
	gsed -i "s#${from_component_pascal}#${to_component_pascal}#g" "${NEW_FILENAME}"
done
